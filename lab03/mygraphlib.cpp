  ...
  for (int i=0;i<nnodes;i++) {
	...
	if (p==0) {   // For example, to read:   node_name   posx   posy
	  ...
	  posx[v]=DBL_MAX;  posy[v]=DBL_MAX; }
        ...
        else if (p==3) { b_vertex[v] = atoi(token.c_str()); } // b-matching
      }}
  }
}
                    ...
                    NodePosMap& posy)
{
  ...
  ReadListGraphNodes(g,n,ifile,string2node,vname,b_vertex,posx,posy);
                   ...
                   NodePosMap& posy)
{
  ...
  else r = ReadListGraph2(filename,g,nodename,b_vertex,custo,posx,posy);
  return(r);
}
...
int ViewListGraph(ListGraph &g,
		...
		EdgeColorMap& ecolor,
		string text){
	NodeStringMap vlabel(g);
	for ( NodeIt v(g); v!=INVALID; ++v )
		vlabel[v] = vname[v];
	return ViewListGraph(g,vname, vlabel, ename, px, py, vcolor, ecolor, text);
}
...
int ViewListGraph(ListGraph &g,
	      ...
	      NodeStringMap &vlabel, // label of the nodes
	      ...
	      string text) // text displayed below the figure
{
  ...
  for (NodeIt v(g); v!=INVALID; ++v) {
    ...
    fprintf(fp,"\t%s [style = \"bold\", color=\"%s\", pos = \"%lf,%lf!\" , label=\"%s\"];\n",
	    vname[v].c_str(),ColorName(vcolor[v]).c_str(),factor*(px[v]-minpx)/delta,factor*(py[v]-minpy)/delta, vlabel[v].c_str());
  }
  int nar=0;
...
int ViewListGraph(ListGraph &g,
	      NodeStringMap &vname, // name of the nodes
	      NodeStringMap &vlabel, // name of the nodes
	      EdgeStringMap &ename,  // name of edges
	      NodeColorMap&    vcolor, // color of node (see myutils.h)
	      EdgeColorMap&    ecolor, // color of edge
	      string text) // text displayed below the figure
